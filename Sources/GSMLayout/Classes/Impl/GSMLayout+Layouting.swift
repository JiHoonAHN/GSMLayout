...
extension GSMLayout {
    ...
    private func apply(onView view: View) {
        ...
        if GSM.autoSizingInProgress, let autoSizeCalculable = view as? AutoSizeCalculable {
    ...
    private func handleGSMEdges() {
        guard shouldGSMEdges else { return }
        guard _top == nil || _bottom == nil || _left == nil || _right == nil else {
            return warn("gsmEdges() won't be applied, top, left, bottom and right coordinates are already set.")
        }
    ...
    private func computeAspectRatio(_ aspectRatio: CGFloat, size: Size) -> Size {
        ...
        guard size.width == nil || size.height == nil else {
            warn("aspectRatio won't be applied, the width and the height are already defined by other GSMLayout's properties.")
            return size
        }
